...
protected:
  ...
  void nextSweep(uint16_t i)
  {
    ...
    {
      case UP:
        DEBUG_PRINT("Index Direction: ");
        DEBUG_PRINTLN(pStripStates_[i].indexDirection);
        for (uint16_t pixel = 0; pixel < n; ++pixel)
        {
          ...
          DEBUG_PRINT((pStripStates_[i].index - (int16_t)pixel));
          DEBUG_PRINT(" ");
          DEBUG_PRINT(((int16_t)pStripStates_[i].pSize0));
        ...
        DEBUG_PRINTLN(pStripStates_[i].index - (int16_t)pStripStates_[i].pSize0);
      ...
      case DOWN:
        ...
        for (uint16_t pixel = 0; pixel < n; ++pixel)
        {
          ...
          //  and pattern size, set to secondary color
          DEBUG_PRINT((int16_t)pixel - pStripStates_[i].index);
          DEBUG_PRINT(" ");
          DEBUG_PRINT(((int16_t)pStripStates_[i].pSize0));
  ...
  void nextBounce(uint16_t i)
  {
    ...
    {
      DEBUG_PRINTLN("Switching to DOWN");
      pStripStates_[i].indexDirection = DOWN;
    }
    else if ((pStripStates_[i].indexDirection == DOWN) &&
             (pStripStates_[i].index <= 0))
    {
      DEBUG_PRINTLN("Switching to UP");
      pStripStates_[i].indexDirection = UP;
    }
    ...
    switch(pStripStates_[i].indexDirection)
    {
      case UP:
        for (uint16_t pixel = 0; pixel < n; ++pixel)
        {
          DEBUG_PRINT(pStripStates_[i].index - (int16_t)pixel);
          DEBUG_PRINT(" % ");
          DEBUG_PRINT((int16_t)pStripStates_[i].pSize0 + (int16_t)pStripStates_[i].pSize1);
          DEBUG_PRINT(" = ");
          DEBUG_PRINT((pStripStates_[i].index - (int16_t)pixel) % ((int16_t)pStripStates_[i].pSize0 + (int16_t)pStripStates_[i].pSize1));
          DEBUG_PRINT(" vs. ");
          DEBUG_PRINT((int16_t)pStripStates_[i].pSize0);
  ...
  void runStrips(void)
  {
    for (uint16_t i = 0; i < NUM_STRIPS; i++)
    {
      ...
      else
      {
        ...
        {
          //DEBUG_PRINTLN("Function Switch Entered...");
          switch (pStripStates_[i].function)
          {
            case OFF:
              fill(pStrips_[i], BLACK);
